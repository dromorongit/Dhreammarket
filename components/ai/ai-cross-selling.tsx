...
function AICrossSellCard({ item }: { item: AICrossSellItem }) {
...
export function AICrossSelling({ entityId, entityType, title, limit }: AICrossSellingProps) {
  const [items, setItems] = useState<AICrossSellItem[]>([])
  ...
  const fetchCrossSell = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/ai/frequently-bought?${params.toString()}`)
      if (response.ok) {
        const data = await response.json()
        setItems(data.frequentlyBought ?? [])
      }
    } catch (error) {
      console.error('Error fetching AI cross-sell:', error)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <section className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-deep-navy mb-6">{title ?? 'Frequently Bought Together'}</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {items.map((item) => (
            <AICrossSellCard key={`${item.type}_${item.id}`} item={item} />
          ))}
        </div>
      </div>
    </section>
