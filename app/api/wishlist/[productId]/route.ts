...
export async function DELETE(request: NextRequest, { params }: { params: { productId: string } }) {
  try {
    ...
    try {
      ...
      console.log('[wishlist/DELETE] wishlist.findUnique succeeded, wishlistId:', wishlist?.id)
    } catch (e) {
    ...
    try {
      ...
      console.log('[wishlist/DELETE] wishlistItem.deleteMany succeeded')
    } catch (e: any) {
      if (e?.code === 'P2025') {
        return NextResponse.json({ error: 'Item not found in wishlist' }, { status: 404 })
      }
    ...
    try {
      ...
      console.log('[wishlist/DELETE] updated wishlist.findUnique succeeded')
    } catch (e) {
