...
function SectionHeader({
  ...
  return (
    <div className="mb-10">
      ...
      {subtitle && (
        <p className={`mt-2 ${dark ? "text-slate-400" : "text-slate-600"}`}>
          {subtitle}
        </p>
      )}
    </div>
...
function EnterpriseSectionSkeleton({ dark = false }: { dark?: boolean }) {
  return (
    <section
      className={`relative py-16 lg:py-24 ${dark ? "bg-slate-900" : "bg-white"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10">
          <div
            className={`h-6 w-32 rounded-full mb-3 ${dark ? "bg-slate-700" : "bg-slate-200"}`}
          />
          <div
            className={`h-8 w-48 rounded ${dark ? "bg-slate-700" : "bg-slate-200"}`}
          />
        </div>
