...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (existing.status !== 'QUOTED') {
      perf.markPrismaEnd(prismaPerfStart)
      perf.log()
      return NextResponse.json({ error: 'No quotation available to accept' }, { status: 400 })
    }
    ...
    await createNotification(
      ...
      'Quotation Accepted',
      `Your quotation for service request "${id}" has been accepted by the customer.`
    )
    ...
    await createNotification(
      ...
      NotificationType.QUOTE_ACCEPTED,
      'Quotation Accepted',
      'You have accepted the vendor quotation. The project is now underway.'
    )

    perf.markPrismaEnd(prismaPerfStart)
    perf.log()
    return NextResponse.json({ message: 'Quotation accepted successfully', request: await getPrisma().serviceRequest.findUnique({ where: { id } }) })
  } catch (error) {
    perf.markPrismaEnd(prismaPerfStart)
    perf.log()
