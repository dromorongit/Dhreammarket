...
export default function NonReturnableBadge({ isReturnable, className, compact }: NonReturnableBadgeProps) {
  ...
  if (compact) {
    return (
      <div className={`inline-flex items-center gap-1 text-rose-700 bg-rose-50 border border-rose-200 rounded px-1.5 py-0.5 ${className || ''}`}>
        ...
        <span className="text-[10px] font-semibold truncate">Non-returnable</span>
      </div>
    )
  }
